(function() {
    ...
    var template = require('underscore'),
        fs = require('fs'),
        helper = require('jsdoc/util/templateHelper'),
        scopeToPunc = { 'static': '.', 'inner': '~', 'instance': '#' };
        
        template.templateSettings.evaluate    = /<\?js([\s\S]+?)\?>/g;
        template.templateSettings.interpolate = /<\?js=([\s\S]+?)\?>/g;
    ...
    exports.publish = function(data, opts) {
        var out = '',
            containerTemplate = template.template(fs.readFileSync(env.dirname + '/templates/JSDoc-DataTables/tmpl/container.tmpl')),
            indexTemplate = template.template(fs.readFileSync(env.dirname + '/templates/JSDoc-DataTables/tmpl/index.tmpl'));
        
        function render(tmpl, partialData) {
            var renderFunction = arguments.callee.cache[tmpl];
            if (!renderFunction) {
                renderFunction = arguments.callee.cache[tmpl] = template.template(fs.readFileSync(env.dirname + '/templates/JSDoc-DataTables/tmpl/'+tmpl));
            }
